...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if ($errors['field-name-2']) {
      ...
      if ($_COOKIE['field-name-2_error'] == '1') {
          $messages[] = '<div class="error">Напишите о себе.</div>';
      }
      else {
          $messages[] = '<div class="error">Пишите на русском языке.</div>';
      }
  }
  ...
  $values['year'] = empty($_COOKIE['year_value']) ? '' : intval($_COOKIE['year_value']);
  $values['sex'] = empty($_COOKIE['sex_value']) ? '' : $_COOKIE['sex_value'];
  $values['email'] = empty($_COOKIE['email_value']) || !(filter_var($_COOKIE['email_value'], FILTER_VALIDATE_EMAIL)) ? '' : $_COOKIE['email_value'];
  $values['Limbs'] = empty($_COOKIE['Limbs_value']) ? '' : $_COOKIE['Limbs_value'];
  $values['field-name-2'] = empty($_COOKIE['field-name-2_value']) || !preg_match('/^[а-яА-Я ]+$/u',$_COOKIE['field-name-2_value']) ? '' : strip_tags($_COOKIE['field-name-2_value']);
  ...
  include('form.php');
    ...
    if (empty($_POST['field-name-2'])) {
        ...
        if (!preg_match('/^[а-яА-Я ]+$/u', $_POST['field-name-2'])) {
            setcookie('field-name-2_error', '2', time() +  24 * 60 * 60);
            $errors = TRUE;
        }
...
// *************
  ...
  if (!empty($_COOKIE[session_name()]) &&
      session_start() && !empty($_SESSION['login'])) {
          ...
          $conn = new PDO('mysql:host=localhost;dbname=u16680', $username, $password,
              array(PDO::ATTR_PERSISTENT => true));//mysqli_connect($servername, $username, $password, $dbname);
          
              try {
                  $stmt = $conn->prepare("UPDATE task6 SET name = ?  where login = ?");
                  $stmt->execute([$_POST['fio'],$_SESSION['login']]);
                  
                  $stmt = $conn->prepare("UPDATE task6 SET year = ?  where login = ?");
                  $stmt->execute([$_POST['year'],$_SESSION['login']]);
                  
                  
                  $stmt = $conn->prepare("UPDATE task6 SET sex = ?  where login = ?");
                  $stmt->execute([$_POST['sex'],$_SESSION['login']]);
                  
                  
                  $stmt = $conn->prepare("UPDATE task6 SET email = ?  where login = ?");
                  $stmt->execute([$_POST['email'],$_SESSION['login']]);
                  
                  
                  $stmt = $conn->prepare("UPDATE task6 SET limbs = ?  where login = ?");
                  $stmt->execute([$_POST['Limbs'],$_SESSION['login']]);
                  
                  
                  $stmt = $conn->prepare("UPDATE task6 SET biography = ?  where login = ?");
                  $stmt->execute([$_POST['fiel-name-2'],$_SESSION['login']]);
                  
                  
                  $stmt = $conn->prepare("UPDATE task6 SET abilities = ?  where login = ?");
                  $stmt->execute([$ability,$_SESSION['login']]);
              }
